...
# 2. 根据板子型号映射WAN和LAN接口（已修改：强行指定 eth1 为 WAN，其余为 LAN）
board_name=$(cat /tmp/sysinfo/board_name 2>/dev/null || echo "unknown")
echo "Board detected: $board_name" >>$LOGFILE
...
# 动态计算 LAN 口：从所有物理网口中剔除 eth1，剩下的全部加入 LAN (如 eth0 eth2 eth3)
lan_ifnames=""
...
echo "Forced mapping applied: WAN=$wan_ifname LAN=$lan_ifnames" >>"$LOGFILE"
